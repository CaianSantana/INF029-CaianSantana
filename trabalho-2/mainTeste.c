...
int main() //obs: rodar teste um por um, eles tem incompatibilidade entre si por conta de criação de mesma estrutura e etc.
{
    inicializar();
    testeInserirSemNada(); //feito
    testeCriarEstrutura(); //feito
    testeInserirComEstrutura(); //feito
    testeExcluir(); //feito
    testeExcluirNumeroEspecifico(); //feito
    testeListar(); //feito
    finalizar();
    inicializar();
    testeRetornarTodosNumeros(); //feito
    testeMudarTamanhoEstrutura(); //feito
    //testeListaEncadeada();
    finalizar();
}
int ligado = 1;
